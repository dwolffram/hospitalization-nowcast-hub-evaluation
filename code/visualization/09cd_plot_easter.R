...
df <- load_truth(as_of = "2022-04-19") %>% 
  filter(location == "DE",
         age_group == "00+") %>% 
...
df_frozen <- read_csv("data/truth_frozen/truth_frozen_0d.csv") %>% 
  filter(location == "DE",
         age_group == "00+") %>% 
...
df_ensemble <- load_nowcast("NowcastHub-MeanEnsemble", "2022-04-19")

ALPHAS <- setNames(c(0.7, 0.4), c("50%", "95%"))

ggplot() +
  geom_line(data = df_all, aes(x = date, y = truth, color = version), size = 1.5) +
  geom_vline(xintercept = as.Date("2022-04-19"), color = "black", linetype = "dashed") +
  geom_ribbon(data = df_ensemble, 
  ...
  geom_ribbon(data = df_ensemble, 
              aes(x = target_end_date, ymin = quantile_0.25, ymax = quantile_0.75, 
                  alpha = "50%"), 
              fill = "skyblue3") +
  geom_line(data = df_ensemble, aes(x = target_end_date, y = quantile_0.5), 
...
df <- load_truth(as_of = "2022-04-20") %>% 
...
ggplot() +
  geom_line(data = df_all, aes(x = date, y = truth, color = version), size = 1.5) +
  geom_vline(xintercept = as.Date("2022-04-20"), color = "black", linetype = "dashed") +
